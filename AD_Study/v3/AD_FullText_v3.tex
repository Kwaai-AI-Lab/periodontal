...
Full details of our model mechanisms are provided in the supplementary information.
...
\begin{table}[htbp]
\centering
\caption{Summary of cumulative PSA results. 2024\pounds\ costs.}
\label{tab:psa}
\small
\begin{tabular}{lccc}
\toprule
...
Total costs (\pounds bn) & 620.6 (516.6--724.4); & 642.2 (529.9--752.9); & 662.6 (544.8--779.6); \\
 & CV: 8.4\% & CV: 8.6\% & 8.9\% \\
Total QALYs (mn) & 151.9 (136.1--167.2); & 152.2 (136.4--167.3); & 152.4 (136.6--167.4); \\
 & CV: 5.3\% & CV: 5.3\% & 5.2\% \\
Incident cases (mn) & 2.7 (2.2--3.2); & 2.8 (2.3--3.3); & 2.9 (2.4--3.5); \\
 & CV: 9.1\% & CV: 9.3\% & 9.7\% \\
\bottomrule
\end{tabular}
\end{table}

The results of our one-way sensitivity analysis are summarised in Table \ref{tab:oneway} (full results in Supplementary Material, Table XXX). Using the lower HR reduced total costs, caregiver QALYs, and incident dementia onsets by 8\%--13\% across scenarios. Total population QALYs were largely insensitive to HR variation, changing by only $\pm$3\% across all scenarios. All results behaved as expected, except the fall in total incident onsets under 25\% PD prevalence when the upper bound HR was used. For all other outputs, using the upper HR led to total costs falling by 1\%--5\% and incident onsets increasing by up to 3\% under the 75\% PD prevalence scenario.
...
\toprule
\textbf{PD prevalence} & \textbf{HR Used} & \makecell{\textbf{Total Costs} \\ \textbf{(\% change from} \\ \textbf{baseline)}} & \makecell{\textbf{Total population} \\ \textbf{QALYs*} \\ \textbf{(\% change from} \\ \textbf{baseline)}} & \makecell{\textbf{Total caregiver} \\ \textbf{QALYs} \\ \textbf{(\% change from} \\ \textbf{baseline)}} & \makecell{\textbf{Total Incident} \\ \textbf{Onsets} \\ \textbf{(\% change from} \\ \textbf{baseline)}} \\
\midrule
75\% & High (1.38) & \pounds643,958,973,993.60 ($-1$) & 136,230,776.09 ($-3$) & 10,981,842.40 (1) & 3,087,083 (3) \\
 & Low (1.07) & \pounds564,567,763,775.41 ($-13$) & 136,915,148.84 ($-3$) & 9,545,927.86 ($-12$) & 2,621,631 ($-13$) \\
50\% & High (1.38) & \pounds611,051,531,957.90 ($-3$) & 136,513,710.72 ($-3$) & 10,394,266.97 ($-1$) & 2,894,042 (0) \\
 & Low (1.07) & \pounds558,137,902,469.89 ($-11$) & 136,968,559.56 ($-3$) & 9,430,100.22 ($-11$) & 2,582,837 ($-11$) \\
25\% & High (1.38) & \pounds578,289,719,287.51 ($-5$) & 136,784,210.03 ($-3$) & 9,799,130.70 ($-4$) & 2,701,735 ($-3$) \\
 & Low (1.07) & \pounds551,576,202,882.79 ($-9$) & 137,010,344.18 ($-3$) & 9,312,173.99 ($-9$) & 2,544,945 ($-8$) \\
\bottomrule
\end{tabular}
\end{table}
...
% Add author affiliations and contact information here
% Example:
% Edward Coote$^{1,*}$, Dr Nitesh Patel$^{2}$, Dr Rajesh Vijayanarayanan$^{3}$
%
...
\section*{Keywords}
% Add 3-5 keywords here
% Suggested: Periodontal disease; Dementia; Microsimulation; Cost-effectiveness; Risk factors; Public health

\section*{Statements \& Declarations}

\subsection*{Funding}
% Declare any funding sources here
% Example: This work was supported by [funding agency] under grant [number].
% Or: No funding was received for conducting this study.

\subsection*{Competing Interests}
% Declare any competing interests here
% Example: The authors have no relevant financial or non-financial interests to disclose.

\subsection*{Author Contributions}
% Describe each author's contribution here
% Example:
% Edward Coote: Conceptualization, Methodology, Software, Formal analysis, Writing - original draft
% Dr Nitesh Patel: Supervision, Writing - review \& editing
% Dr Rajesh Vijayanarayanan: Supervision, Writing - review \& editing
...
% Example: The model code and data used in this study are available at [repository URL].
